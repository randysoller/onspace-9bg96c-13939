          ...
          <svg width="100" height="135" viewBox="0 0 100 135" className="select-none">
            ...
            {chord.frets.map((fret, idx) => {
              if (fret === -1) {
                ...
                if (isRoot) {
                  // Open root note - blue diamond (5% increase: 5.67)
                  return (
                    <path
                      key={`marker-${idx}`}
                      d={`M ${10 + idx * 16} ${12 - 5.67} 
                          L ${10 + idx * 16 + 5.67} ${12} 
                          L ${10 + idx * 16} ${12 + 5.67} 
                          L ${10 + idx * 16 - 5.67} ${12} Z`}
                      fill="none"
                      stroke="currentColor"
                      strokeWidth="2"
            ...
            {chord.frets.map((fret, stringIdx) => {
              if (fret > 0) {
                ...
                if (isRoot) {
                  // Root note - blue diamond (5% increase: 10.395)
                  return (
                    <g key={`dot-${stringIdx}`}>
                      <path
                        d={`M ${10 + stringIdx * 16} ${20 + (fret - 0.5) * 25 - 10.395} 
                            L ${10 + stringIdx * 16 + 10.395} ${20 + (fret - 0.5) * 25} 
                            L ${10 + stringIdx * 16} ${20 + (fret - 0.5) * 25 + 10.395} 
                            L ${10 + stringIdx * 16 - 10.395} ${20 + (fret - 0.5) * 25} Z`}
                        fill="currentColor"
                        className="text-cyan-500"
                      />
