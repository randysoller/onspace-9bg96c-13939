                ...
                <svg width="240" height="300" viewBox="0 0 240 300" className="select-none">
                  ...
                  {currentChord.frets.map((fret, idx) => {
                    ...
                    if (fret === -1) {
                      ...
                      if (isRoot) {
                        // Open root note - blue diamond (5% reduction: 8.9775 * 0.95 = 8.528625)
                        return (
                          <path
                            key={`open-${idx}`}
                            d={`M ${60 + idx * 30} ${40 - 8.528625} 
                                L ${60 + idx * 30 + 8.528625} ${40} 
                                L ${60 + idx * 30} ${40 + 8.528625} 
                                L ${60 + idx * 30 - 8.528625} ${40} Z`}
                            fill="none"
                            stroke="currentColor"
                            strokeWidth="2.5"
                    ...
                    if (fret > 0) {
                      ...
                      if (isRoot) {
                        return (
                          <g key={`dot-${stringIdx}`}>
                            <path
                              d={`M ${60 + stringIdx * 30} ${60 + (fret - 0.5) * 50 - 12.319125} 
                                  L ${60 + stringIdx * 30 + 12.319125} ${60 + (fret - 0.5) * 50} 
                                  L ${60 + stringIdx * 30} ${60 + (fret - 0.5) * 50 + 12.319125} 
                                  L ${60 + stringIdx * 30 - 12.319125} ${60 + (fret - 0.5) * 50} Z`}
                              fill="currentColor"
                              className="text-cyan-500"
                            />
